...
def save_attendance(request):
    ...
    try:
        ...
        session = get_object_or_404(Session, id=session_id)
        subject = get_object_or_404(Subject, id=subject_id)
        attendance = Attendance(section=section, session=session, subject=subject, date=date)
        attendance.save()
...
def get_attendance(request):
    ...
    try:
        ...
        section = get_object_or_404(Section, id=section_id)
        session = get_object_or_404(Session, id=session_id)
        attendance = Attendance.objects.filter(subject=subject,section=section, session=session)
        attendance_list = []
...
def staff_apply_leave(request):
    ...
    if request.method == 'POST':
        form = LeaveReportStaffForm(request.POST, request.FILES)
